...
class Model:
    def __init__(self, model_name: str, provider: str = 'openai'):
        ...
        if provider == 'huggingface':
            ...
            self.client = OpenAI(
                ...
                timeout=300,
                max_retries=0  # 우리가 직접 재시도 로직을 처리
            )
    ...
    def query(self, prompt: str, **kwargs) -> Union[str, list]:
        ...
        elif self.provider == 'openai_ollama':
            return self.query_openai_ollama(prompt, **kwargs)
    ...
    def query_with_timeout(self, messages, **kwargs):
        if self.provider == 'openai_ollama':
    ...
    def query_openai(self, 
        ...
        for i in range(64):
            try:
                ...
                print(f"[Retry {i+1}/64] OpenAI API Error: {e}")
                time.sleep(10)

        raise RuntimeError("Failed to query the OpenAI API after 64 retries.")
    
    def query_openai_ollama(self, 
                     prompt: str, 
                     system: Optional[str] = None, 
                     rate_limit_per_minute: Optional[int] = None, **kwargs) -> Union[str, list]:
        # Set default system message
        ...
        for i in range(64):
            try:
                ...
                error_str = str(e)
                print(f"[Retry {i+1}/64] Ollama API Error: {e}")
                
                # 다양한 재시도 전략
                if "500" in error_str:
                    print("  -> 500 error, waiting 3 seconds...")
                    time.sleep(3)
        ...
        raise RuntimeError("Failed to query the Ollama API after 64 retries.")
