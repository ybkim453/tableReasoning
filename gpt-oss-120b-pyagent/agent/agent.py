...
class TableAgent:
    ...
    def _gen_budget(self, prompt_length: int, cap: int = None) -> int:
        if cap is None:
            import os
            cap = int(os.getenv("GEN_CAP", "1024"))
        available = 32768 - prompt_length - 1024
        return int(max(64, min(cap, max(0, available))))
        ...
        if isinstance(self.model, Model):
            if prompt_length <= 3328:
                text, response = self.model.query(
                    ...
                    top_p=self.top_p,
                    max_tokens = self._gen_budget(prompt_length),
