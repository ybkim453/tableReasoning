...
def main(
        model:Optional[str] = "deepseek-r1:70b", # base model of the agent (for short prompt to save money)
        long_model:Optional[str] = "deepseek-r1:70b", # long model of the agent (only used for long prompt)
    ...
    temp = []
    temp_path = "output/wtq_agent_wo_norm/result.jsonl"
    if os.path.exists(temp_path):
        with open(temp_path, "r") as f:
            temp = [json.loads(line) for line in f if line.strip()]
    else:
        print(f"⚠ Cache file not found: {temp_path} — skipping cache reuse.")
    ...
    for table_idx, d in enumerate(data):
        ...
        for idx in index_list:
            ...
            # 캐시 재사용 조건: transpose_flag가 False이고 temp가 충분히 있을 때만
            if not transpose_flag and global_i < len(temp):
